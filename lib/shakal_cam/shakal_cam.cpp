...
// Send PNG image (8-bit grayscale, 320x213 or 320x240, cropped to 3:2 if needed)
void sendNewPNG(camera_fb_t *fb, WiFiClient &client) {
    ...
    ensureBigPixelBuf(width * height);
    makeBigPixels8x8(fb->buf, width, height, g_bigPixelBuf);
    const uint8_t* pngBuf = g_bigPixelBuf;
    int pngWidth = width;
    ...
    size_t outsize = 0;
    unsigned error = lodepng_encode_memory(&out, &outsize, pngBuf, pngWidth, pngHeight, LCT_GREY, 8);
    if (error) {
...
// Send PNG image (8-color palette, 320x213 or 320x240, cropped to 3:2 if needed)
void sendNewPNGWithPalette(camera_fb_t *fb, WiFiClient &client) {
    int width = fb->width;
    int height = fb->height;
    bool crop3_2 = (width == 320 && height == 240); // QVGA only
    int cropHeight = crop3_2 ? 213 : height;
    // ВЕРНУТА пикселизация: используем makeBigPixels8x8
    ensureBigPixelBuf(width * height);
    makeBigPixels8x8(fb->buf, width, height, g_bigPixelBuf);
    ...
    // Индексируем пиксели по диапазонам (0-7)
...
void makeBigPixels8x8(const uint8_t* src, int srcWidth, int srcHeight, uint8_t* dst) {
    const int blockSize = 4; // Было 8, теперь 4
